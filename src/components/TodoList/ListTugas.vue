   ...
   <v-card>
     ...
     <v-data-table :headers="headers" :items="todos" :search="search"  >
       ...
       <template v-slot:[`item.actions`]="{ item }">   
        <v-btn icon color="green" @click="detailItem(item)">
    ...
    <v-dialog v-model="batalHapus" max-width="500px">
        <v-card>
            <v-card-title>Apakah anda yakin menghapus?</v-card-title>
            <v-card-actions>
              <v-btn color="green" text @click="batalHapus = false">Batal</v-btn>
              <v-btn color="red" text @click="Hapus">Iya</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
...
export default {
 ...
 methods: {
    deleteItem(item){
        ...
        this.batalHapus = true
    },
    Hapus(item) {
        this.todos.splice(this.index,1)
        this.batalHapus = false
    },
    detailItem(item){
        this.detail = {
                task: item.task,
